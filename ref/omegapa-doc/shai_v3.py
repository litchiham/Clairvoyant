import os
os.environ["OMEGA_BIN_PATH"] = r"D:\PlanetaryDataSystem\OMEGA\bin"
os.environ["OMEGA_PY_PATH"] = r"D:\PlanetaryDataSystem\OMEGA\py" #这个似乎没用
...
# ====== 配置路径 ======
DATA_ROOT = r"D:\PlanetaryDataSystem\OMEGA\bin"   # 每个子文件夹都有 .nav/.qub 和 reference_files
DUST_PATH = r"D:\PlanetaryDataSystem\krigedcdod_v2\*.nc"  # 所有 dust nc 文件
OUTPUT_DIR_No = r"D:\PlanetaryDataSystem\OMEGA\py\Omega_after_no"    #不需要处理个别像素的文件
OUTPUT_DIR_clean_cube =r"D:\PlanetaryDataSystem\OMEGA\py\Omega_after_yes"    #已经经过处理个别像素的文件
OUTPUT_DIR_rejected=r"D:\PlanetaryDataSystem\OMEGA\py\Omega_after_rejected"  
...
def process_cube(qub_path):
    try:
        ...
        cube_corr = od.corr_therm_atm(cube)
        # cube_corr=cube #测试

        cube, key = clean_cube(cube)
...
'''
    更新日志：
    v2: 修复多线程，添加中断机制
    v3: 确定筛选机制大约没有问题，确定最适宜并行数~16，确定可以舍弃therm校准
'''
...
'''
    ...
    3. 小规模测试：v3，26轨道，并行数10，总用时00:33，平均1.26s/个
'''