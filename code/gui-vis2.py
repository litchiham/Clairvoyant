...
class SimpleSphereViewer:
    def __init__(self):
        ...
        self.predicted_files = []  # 存储检测到的文件列表
        cio.cubeio = None  # CubeIO实例
        
        # 创建界面
        self.create_widgets()
        
        # 初始化CubeIO并扫描predicted目录
    ...
    def scan_predicted_files(self):
        ...
        try:
            ...
            if not os.path.exists(predicted_dir):
                print(f"predicted目录不存在: {predicted_dir}")
                self.status_label.config(text=f"目录不存在: {predicted_dir}")
                return
            
            # 查找所有.pkl文件
            pattern = os.path.join(predicted_dir, '*_predicted.pkl')
            files = glob.glob(pattern)
            
            self.predicted_files = []
            for filepath in files:
                filename = os.path.basename(filepath)
                # 提取cube名称，例如: "0982_3_predicted.pkl" -> "0982_3"
                if filename.endswith('_predicted.pkl'):
                    cube_name = filename[:-len('_predicted.pkl')]
                    file_size = os.path.getsize(filepath)
                    file_size_mb = file_size / (1024 * 1024)
                    
                    self.predicted_files.append({
                        'path': filepath,
                        'filename': filename,
                        'cube_name': cube_name,
            ...
            print(f"扫描到 {len(self.predicted_files)} 个predicted文件")
            self.status_label.config(text=f"找到 {len(self.predicted_files)} 个数据文件")
    ...
    def update_file_listbox(self):
        ...
        if self.file_listbox:
            ...
            for file_info in self.predicted_files:
    ...
    def create_widgets(self):
        ...
        # 左侧栏标题
        left_title = tk.Label(left_frame, text="Predicted Data Files", font=("Arial", 12, "bold"))
        left_title.pack(side=tk.TOP, fill=tk.X, padx=10, pady=10)
        
        # 刷新按钮
        refresh_btn = tk.Button(left_frame, text="Refresh List", command=self.scan_predicted_files)
    ...
    def load_file_by_index(self, index):
        """根据索引加载文件"""
        if index < 0 or index >= len(self.predicted_files):
            self.status_label.config(text="无效的文件索引")
            return
        
        file_info = self.predicted_files[index]
        ...
        try:
            ...
            # 加载predicted类型的数据
            self.data = cio.cubeio.load(cube_name=cube_name, type='predicted')
    ...
    def project_data(self):
        ...
        try:
            ...
            lat = self.data.lat
            lon = self.data.lon
            # 对于predicted数据，使用reg属性而不是cube_rf
            if hasattr(self.data, 'cube_rf'):
                # processed数据
                cube_rf = self.data.cube_rf
                reflectance = cube_rf[:, :, 0]
            elif hasattr(self.data, 'reg'):
                # predicted数据
                reflectance = self.data.reg
            else:
                raise ValueError("Data object missing required reflectance field (cube_rf or reg)")
            
            if lat is None or lon is None:
                raise ValueError("Data missing required lat/lon fields")
            ...
            if lat.ndim == 2:
                ...
                if grid_size * grid_size != n_points:
                    ...
                    # 填充剩余空间为NaN
                    lat_padded = np.full((ny, nx), np.nan)
                    lon_padded = np.full((ny, nx), np.nan)
                    reflectance_padded = np.full((ny, nx), np.nan)
                    
                    lat_padded.flat[:n_points] = lat
                    lon_padded.flat[:n_points] = lon
                    reflectance_padded.flat[:n_points] = reflectance
                    
                    lat, lon, reflectance = lat_padded, lon_padded, reflectance_padded
                else:
                    ny, nx = grid_size, grid_size
                    lat = lat.reshape(ny, nx)
                    lon = lon.reshape(ny, nx)
                    reflectance = reflectance.reshape(ny, nx)
            else:
                raise ValueError(f"Unsupported lat dimensionality: {lat.ndim}")
            ...
            if valid_mask.any():
                lat_min, lat_max = lat[valid_mask].min(), lat[valid_mask].max()
                lon_min, lon_max = lon[valid_mask].min(), lon[valid_mask].max()
            else:
            ...
            # --- 右图：数据投影（2D）---
            # 创建2D投影：使用经纬度网格显示反射率
            # 使用pcolormesh进行2D投影，处理NaN值
            import matplotlib.pyplot as plt
            
            # 方法1：尝试使用shading='flat'和掩码数组
            try:
                # 创建掩码数组来处理NaN值
                lon_masked = np.ma.masked_invalid(lon)
                lat_masked = np.ma.masked_invalid(lat)
                reflectance_masked = np.ma.masked_invalid(reflectance)
                
                im = ax2.pcolormesh(lon_masked, lat_masked, reflectance_masked, 
                                   cmap=cmap, shading='flat', vmin=-0.02, vmax=0.07)
            except ValueError:
                # 方法2：如果掩码数组失败，使用contourf作为替代
                print("Using contourf as fallback for pcolormesh with NaN values")
                # 创建有效数据的网格
                valid_mask = ~np.isnan(lon) & ~np.isnan(lat) & ~np.isnan(reflectance)
                if valid_mask.any():
                    ...
                    lon_grid, lat_grid = np.meshgrid(
                        np.linspace(lon_valid.min(), lon_valid.max(), 100),
                        np.linspace(lat_valid.min(), lat_valid.max(), 100)
                    )
                    reflectance_grid = griddata(
                        (lon_valid, lat_valid), reflectance_valid, 
                        (lon_grid, lat_grid), method='linear'
                    )
                    
                    im = ax2.contourf(lon_grid, lat_grid, reflectance_grid, 
                                     cmap=cmap, levels=20, vmin=-0.02, vmax=0.07)
                else:
                    # 如果没有有效数据，创建一个空的图像
                    im = ax2.imshow([[0]], cmap=cmap, vmin=-0.02, vmax=0.07)
            ...
            if hasattr(self.data, 'cube_rf'):
                ...
                self.colorbar = self.fig.colorbar(im, ax=ax2, label='Prediction Value')
            
            # 设置右图属性
            if hasattr(self.data, 'cube_rf'):
                ax2.set_title("OMEGA Data Projected (2D)")
            else:
                ax2.set_title("Predicted Data Projected (2D)")
    ...
    def find_nearest_pixel_2d(self, click_lon, click_lat):
        ...
        min_valid_idx = np.argmin(valid_distances)
        
        # 将有效索引转换回原始数组索引
        valid_indices = np.where(valid_mask.flat)[0]
        min_idx_flat = valid_indices[min_valid_idx]
        min_idx = np.unravel_index(min_idx_flat, distances.shape)
        i, j = min_idx
    ...
    def on_click(self, event):
        ...
        try:
            ...
            # 获取波长和反射率数据
            if OMEGAPY_AVAILABLE and hasattr(self.data, 'lam') and hasattr(self.data, 'cube_rf'):
                # processed数据 (OMEGAdata对象)
                wavelengths = self.data.lam
                reflectance = self.data.cube_rf[i, j, :]
            elif hasattr(self.data, 'reg'):
                # predicted数据 (Predicted对象)
                print("点击事件：预测数据不支持光谱显示")
                self.status_label.config(text=f"Prediction value at ({lat:.1f}°, {lon:.1f}°): {self.data.reg[i, j]:.4f}")
                return
            else:
